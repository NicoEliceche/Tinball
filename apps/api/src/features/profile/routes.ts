...
export async function profileRoutes(app: FastifyInstance) {
  ...
  app.put('/api/v1/profile/onboarding', { preHandler: [authenticate, mutationRateLimit] }, async (request, reply) => {
    ...
    const user = await prisma.$transaction(async (tx) => {
      await tx.playerProfile.upsert({ where: { userId: request.auth!.userId }, create: { userId: request.auth!.userId, birthDate, nickname: input.nickname.trim() || null, locality: input.locality, province: input.province, primaryPosition: input.primaryPosition, secondaryPositions: input.secondaryPositions, skillLevel: input.skillLevel, preferredFoot: input.preferredFoot, bio: input.bio }, update: { birthDate, nickname: input.nickname.trim() || null, locality: input.locality, province: input.province, primaryPosition: input.primaryPosition, secondaryPositions: input.secondaryPositions, skillLevel: input.skillLevel, preferredFoot: input.preferredFoot, bio: input.bio } });
