...
import { BioField, Chip, Chips, ChipText, Copy, ErrorText, Field, FormSection, Helper, Intro, Label, Step, Title } from './OnboardingScreenStyled';
...
export function OnboardingScreen({ editing = false, onSaved }: { editing?: boolean; onSaved?: () => void }) {
  ...
  return (
    <ScrollScreen>
      ...
      <FormSection><Label>Posición principal</Label><Chips>{positions.map((item) => <Chip key={item.value} $selected={position === item.value} accessibilityRole="button" accessibilityState={{ selected: position === item.value }} onPress={() => setPosition(item.value)}><ChipText $selected={position === item.value}>{item.label}</ChipText></Chip>)}</Chips></FormSection>
      <FormSection>
      ...
      <FormSection><Label>Nivel</Label><Chips>{levels.map((item) => <Chip key={item.value} $selected={level === item.value} accessibilityRole="button" accessibilityState={{ selected: level === item.value }} onPress={() => setLevel(item.value)}><ChipText $selected={level === item.value}>{item.label}</ChipText></Chip>)}</Chips></FormSection>
      <FormSection><Label>Pie hábil</Label><Chips>{feet.map((item) => <Chip key={item.value} $selected={preferredFoot === item.value} accessibilityRole="button" accessibilityState={{ selected: preferredFoot === item.value }} onPress={() => setPreferredFoot(item.value)}><ChipText $selected={preferredFoot === item.value}>{item.label}</ChipText></Chip>)}</Chips></FormSection>
      <FormSection>
